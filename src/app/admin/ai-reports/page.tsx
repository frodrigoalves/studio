...
import { Loader2, Sparkles, Wand2, FileText, Upload, Lightbulb, ListChecks, BarChart, Archive, BrainCircuit, GaugeCircle, AlertTriangle, Fuel, DollarSign, LineChart, BarChart2 } from "lucide-react";
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                // For Excel files, it might be an ArrayBuffer
                if (file.type.includes('spreadsheet') || file.type.includes('excel') || file.name.endsWith('.xlsx') || file.name.endsWith('.xls')) {
                    ...
                    // Pass the CSV text as a data URI
                    resolve(`data:text/csv;base64,${btoa(csvText)}`);
                } else if (file.type === 'text/csv' || file.name.endsWith('.csv')) {
                     // For CSV, read as text and convert to data URI
                    ...
                    // For OCR (images, pdf) we need the full data URI
        ...
        if (file.type.includes('spreadsheet') || file.type.includes('excel') || file.name.endsWith('.xlsx') || file.name.endsWith('.xls')) {
            ...
            // For other file types, let the other handlers manage it.
